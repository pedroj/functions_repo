...
fit.disp <- function
    (x, d2 = NULL, y = NULL, fun = "exponential", pdf = NULL, CDF = NULL,
     np = 1, p = rep(0.5, np))
{
    if (fun == "exponential") {
        dfun <- dexp
    ...
    if (fun == "half-normal") {
        dfun <- function(x, sigma) 2*dnorm(abs(x), mean = 0, sd = sigma)
        pfun <- function(x, sigma) 2*(pnorm(abs(x), mean = 0, sd = sigma) - .5)
        np <- 1
    }
    if (fun == "half-cauchy") {
        dfun <- function(x, sigma) 2*dcauchy(abs(x), location = 0, scale = sigma)
        pfun <- function(x, sigma) 2*(pcauchy(abs(x), location = 0, scale = sigma) - .5)
        np <- 1
    }
    if (fun == "half-t") {
    ...
    if(is.null(d2) & is.null(y) & is.vector(x)) {
        ...
        pdf.arg <- paste("d1,", paste("p[", 1:np, "]", sep = "", collapse=", "))

        ## prepare the pdf function (as an R expression)
        toto.exp <- parse(text=paste("dfun(", pdf.arg, ")", sep="", collapse=""))
    }
    ...
    if(is.null(d2) & is.vector(y) & is.vector(x)) {
        ...
        if(is.numeric(y)) {
            ...
            for (i in 1:n) ifelse(y[i] == val.y[1], dum[i] <- 0, dum[i] <- 1)
        }
        ...
        fun.arg <- paste("d1,", paste("p[", 1:np, "]", sep = "", collapse=", "))

        ## prepare the R expression for the deviance function
        trm1 <- paste("(1 - dum) * dfun(", fun.arg, ")", sep="", collapse="")
        trm2 <- paste("dum * (1 - pfun(", fun.arg, "))", sep="", collapse="")
        toto.exp <- parse(text=paste(trm1, "+", trm2, sep="", collapse=""))
    }
    if(is.vector(d2) & is.vector(y) & is.vector(x) |
        ...
        ## for the pdf and the CDF functions
        fun.arg <- paste("d1,", paste("p[", 1:np, "]", sep = "", collapse=", "))
        fun.arg.bis <- paste("d2,", paste("p[", 1:np, "]", sep = "", collapse=", "))

        ## prepare the R expression for the deviance function
        trm1 <- paste("dum[,1] * dfun(", fun.arg, ")", sep="", collapse="")
        trm2 <- paste("dum[,2] * (pfun(", fun.arg.bis, ") - pfun(", fun.arg, "))",
                      sep="", collapse="")
        trm3 <- paste("dum[,3] * (1 - pfun(", fun.arg, "))", sep="", collapse="")
        toto.exp <- parse(text=paste(trm1, "+", trm2, "+", trm3, sep="", collapse=""))
    }
